...
function crearTarjeta(personaje) {
    ...
    if (personaje.status === "Alive") claseEstado = "activo";
    if (personaje.status === "Dead") claseEstado = "fallecido";

    const estadoTraducido = personaje.status === "Alive" ? "Activo" : personaje.status === "Dead" ? "Fallecido" : "Desconocido";
